...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jltunik_subscribe', array( $this, 'jltunik_subscribe' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltunik_subscribe() {
			check_ajax_referer( 'jltunik_subscribe_nonce' );
		...
		public function notice_content() {
			...
			?>
			<div class="notice notice-jltunik is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> jltunik-notice-<?php echo esc_attr( $this->get_id() ); ?>">
				<button type="button" class="notice-dismiss jltunik-notice-dismiss"></button>
				<img width="70" height="70" src="<?php echo esc_url( JLTUNIK_IMAGES . '/logo.png' ); ?>" alt="<?php esc_attr_e( 'Logo', 'unik-ultimate-pricing-table' ); ?>">
				<div class="jltunik-subscribe-content">
					<?php $this->set_title(); ?>
					<form style="display:flex">
						<div class="jltunik-plugin-subscribe-input">
						...
						<button type="submit" class="button button-primary jltunik-subscribe-btn"><?php esc_html_e( 'Get Discount', 'unik-ultimate-pricing-table' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery(document).on('submit', '.jltunik-notice-subscribe .jltunik-subscribe-content form', function(e) {
					...
					let formWrapper = form.closest('.jltunik-subscribe-content');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jltunik_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jltunik_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							formWrapper.hide().after('<p class="jltunik--notice-message"><strong>' + response.data + '</strong><p>');
							let subsTimeout = setTimeout(function() {
								jltunik_notice_action(null, form, 'disable');
