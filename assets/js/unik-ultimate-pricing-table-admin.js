(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltunik-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltunik-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltunik-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltunik-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTUNIKCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltunik_recommended_activate_plugin",
            file: file,
            nonce: JLTUNIKCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltunik_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTUNIKCORE.recommended_nonce
        },
