...
use JLTUNIK\JLT_Unik_Pricing_Table;
...
if ( ! class_exists( 'Upgrader' ) ) {
	...
	class Upgrader {
		...
		public function __construct() {
			$this->option_name = JLT_Unik_Pricing_Table::plugin_version_key();
		}
		...
		protected function get_installed_version() {
			return get_option( $this->option_name, JLTUNIK_VER );
		}
		...
		public function if_updates_available() {
			if ( version_compare( $this->get_installed_version(), JLTUNIK_VER, '<' ) ) {
				return true;
			}
		...
		public function run_updates() {
			...
			$path              = trailingslashit( JLTUNIK_DIR );
